...
test.describe('Form layouts page', () =>{
  ...
  test("Radio Button", async ({ page }) => {
    ...
    const radioStatus = await usingTheGridForm.getByRole("radio", {
      name: "Option 1",
    }).isChecked();
 ...
 for (const box of await allBoxes.all()) {
  ...
  test("lists and dropdown", async ({ page }) => {
    ...
    //check color theme after change should be change header color
    await dropDownMenu.click();
  ...
  test("tooltips", async ({ page }) => {
    ...
    //chose block with nesessary tooltips
    const toolTipCard = page.locator("nb-card", {
      hasText: "Tooltip Placements",
    });
    //chose button nesessary
  ...
  test("sliders", async ({ page }) => {
    ...
    // create event click for change temperature
